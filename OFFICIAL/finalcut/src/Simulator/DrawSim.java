...
public class DrawSim extends JPanel implements Runnable {
	...
	public DrawSim() {
		...
		for (int i = 0; i < 15; i++) {
			Factoryforce.add(new Factory(i, 220+(i*43), 55));
		}
		
		for (int i = 15; i < 30; i++) {
			Factoryforce.add(new Factory(i, 822-((i-15)*43), 135));
		}

		CDCforce = new CDC(180, 440);
		...
		while (Thread.currentThread() == runner) {
			...
			for (Factory x : Factoryforce) {
				...
				if (x.getminutedemand(Clock.returnminute()) > 0) {
					// OWNSELF TAKE NOTE AND NOTIFY CENTRAL AT THE SAME TIME
					// [DELIVER LOCATION: CDC (30) (Default)]
					...
					if (constant.Truckpriority == true) {
						...
						if (notsuccess == true) {
							...
							if (found == true && potentialtype == 't') {
								// ALLOCATE DESTINATION TO AGV
								Truckforce.get(potential).setdestination(30);
							...
							if (found == true && potentialtype == 'a') {
								// ALLOCATE DESTINATION TO AGV
								Agvforce.get(potential).setdestination(30);
			...
			if (constant.TruckMode == true) {
				for (Truck x : Truckforce) {
					...
					if (x.getphase() == 1) {
						...
						if (x.position.x == Central.factorylocation[x.getdestination()].x && x.position.y == Central.factorylocation[x.getdestination()].y)
								{
							System.out.println("heLLo");
							for (int i = 0; i < constant.Factbay; i++) {
								System.out.println("COMANY" + Central.factdock[x.getdestination()][i]);
								if (Central.factdock[x.getdestination()][i] == 0) {

									// ENTER THE FACTORY
									Central.factdock[x.getdestination()][i] = 1;
									x.setbay(i);
									
									// RMB TO ADD DIST
									x.setloadingclock(Clock.time, 't');

									x.setpreviousphase(x.getphase());
									x.setphase(6);
									System.out.println(x.getphase());
									break;
								}
							}
						}

						if (x.position != Central.factorylocation[x.getdestination()]) {
							// CALCULATE NEW LOCATION
							x.accelerate();
							//System.out.println(x.returnvelo());
							
							distance =x.calcdistance();
							x.move(distance, x.getdestination());
						}

						// Fuel
						x.addfuel((constant.Truckfewocargo/1000) * (distance/constant.scale));
						Central.totalfuel+=(constant.Truckfewocargo/1000) * (distance/constant.scale);
					}
					if (x.getphase() == 2) {
						//System.out.println(x.position);
						if (x.position.x == Central.factorylocation[x.getdestination()].x && x.position.y == Central.factorylocation[x.getdestination()].y)
